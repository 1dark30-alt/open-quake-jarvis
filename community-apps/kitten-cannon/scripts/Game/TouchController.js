...
export const TOUCH_INFORMATION = {
    ...
    // true while a finger / mouse button is held down; position keeps following
    // the pointer, so held on-screen buttons can repeat their action per frame.
    isDown: false,
};
addEventListener("touchend", (event) => {
    TOUCH_INFORMATION.eventType = TOUCH_EVENT_TYPES.up;
    TOUCH_INFORMATION.isDown = false;
});
...
addEventListener("mousemove", (event) => {
    if (!TOUCH_INFORMATION.isDown) return;
    TOUCH_INFORMATION.position.x = event.clientX;
    TOUCH_INFORMATION.position.y = event.clientY;
});
