...
PLUGIN = {
    ...
    "description": (
        ...
        "or showing right now — e.g. 'how many calories are on this plate', "
        "'how many calories is this', 'what's the nutritional value of this'. "
        "This tool takes its OWN "
...
def _analyze(photo: np.ndarray, query: str, api_key: str) -> dict:
    ...
    resp = gemini.call(
        [gtypes.Part.from_bytes(data=jpg.tobytes(), mime_type="image/jpeg"), prompt],
        tier=_TIER, timeout_ms=30_000, key=api_key,
    )
    if resp is None:
...
# ── daily calorie log + rolling memory note ──────────────────────────────────

CAL_STATE = BASE_DIR / "memory" / "calorie_state.json"


def _load_cal() -> dict:
    from datetime import date
    try:
        d = json.loads(CAL_STATE.read_text(encoding="utf-8"))
    except Exception:
        d = {}
    today = date.today().isoformat()
    if d.get("date") != today:
        d = {"date": today, "kcal": 0, "items": []}
    d.setdefault("kcal", 0)
    d.setdefault("items", [])
    return d
...
def _remember_cal(d: dict) -> None:
    """Overwrite ONE rolling memory note with today's calorie total, so JARVIS can
    answer 'how many calories today' in a later conversation. Fixed key → replaces
    the previous value each time (memory never accumulates day after day)."""
    try:
        from memory.memory_manager import remember
        items = ", ".join(d.get("items", [])[-6:])
        note = f"Ate about {d['kcal']} kcal so far on {d['date']}"
        if items:
            note += f" ({items})"
        remember("calories_today", note + ".", "notes")
    except Exception:
        pass


def _record_cal(kcal, food: str):
    """Add this scan to today's running total. Returns the new total or None."""
    n = _num(kcal)
    if n is None:
        return None
    d = _load_cal()
    d["kcal"] = round(d.get("kcal", 0) + n)
    if food:
        d["items"] = (d.get("items", []) + [str(food)])[-20:]
    try:
        CAL_STATE.parent.mkdir(parents=True, exist_ok=True)
        CAL_STATE.write_text(json.dumps(d, ensure_ascii=False), encoding="utf-8")
    except Exception:
        pass
    _remember_cal(d)
    return d["kcal"]
...
def run(parameters: dict, player=None, session_memory=None) -> str:
    ...
    if data.get("food"):
        total = _record_cal(data.get("calories_kcal"), data.get("food"))
        panel = (data.get("panel_text") or "").strip()
        if total is not None:
            line = f"📊 Today so far: ≈{total} kcal"
            panel = f"{panel}\n\n{line}" if panel else line
        if player and panel:
            try:
