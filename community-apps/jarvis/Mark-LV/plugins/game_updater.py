...
# Every field is optional. Leave them alone and this behaves exactly as it did
# as a built-in action; fill one in and it stops guessing.
_NS = "game_updater"
...
PLUGIN_SETTINGS = {
    ...
    "fields": [
        {"key": "install_drive", "label": "Install games to drive", "type": "text",
         "placeholder": "D   —   leave empty to use whichever drive has most room"},
...
def _find_best_drive() -> dict | None:
    ...
    # An explicit choice in plugin settings beats the guess. The guess is only
    # ever "whichever drive has the most room free" — right often enough to keep
    # as the default, wrong often enough to be worth letting the user say.
    chosen = str(_setting("install_drive", "")).strip(" :/\\").upper()[:1]
    if chosen:
        picked = next((d for d in drives if d["letter"] == chosen), None)
        if picked:
            print(f"[GameUpdater] 📌 Install drive from settings: {chosen}:")
            return picked
        print(f"[GameUpdater] ⚠️ Settings ask for drive {chosen}:, which is not "
              f"present — falling back to the drive with most room")

    return max(drives, key=lambda d: d["free_gb"])
...
def game_updater(parameters: dict, player=None, speak=None) -> str:
    ...
    # What the user says in the moment always wins; settings only decide what
    # happens when they say nothing.
    hour      = int(p.get("hour",   _setting_int("schedule_hour", 3)))
    minute    = int(p.get("minute", _setting_int("schedule_minute", 0)))
    _shutdown = p.get("shutdown_when_done")
    shutdown  = (bool(_setting("shutdown_when_done", False)) if _shutdown is None
                 else str(_shutdown).lower() == "true")
