...
to YouTube Studio, using a fixed, always-identical sequence of steps — same
...
SEO-keyword-rich YouTube caption in the user's own language before the
...
Flow (fixed): Opera GX → new tab → youtube.com/upload → click
...
PLUGIN = {
    "name": "upload_video",
    "description": (
        "Uploads the video file named 'video' on the user's Desktop to YOUTUBE "
        "with a fully automated browser flow (Opera GX + YouTube Studio). Use "
        "when the user asks to upload/post/share their video to YouTube — e.g. "
        "'upload this video to YouTube', 'post my video to YouTube'. "
        ...
        "browser_control or computer_control for YouTube uploads."
    ),
    "parameters": {
        "type": "OBJECT",
        "properties": {
            "title": {
                "type": "STRING",
                "description": ("A concise, catchy VIDEO TITLE in the user's own "
                                "language — this becomes the YouTube video title. "
                                "One short line; you may craft it from the user's "
                                "topic (unlike the caption, the title is not "
                                "expanded by the tool)."),
            },
            "description": {
                "type": "STRING",
...
def _generate_caption(brief: str) -> str:
    """Expands the user's rough brief into a LONG, SEO-heavy YouTube caption
    ...
    try:
        ...
        prompt = (
            "You are a YouTube SEO copywriter. The user gives you only a rough "
        ...
        caption = gemini.text(prompt, tier=gemini.SMART, timeout_ms=30_000)
...
def _run_upload_flow(title: str, brief: str, caption_holder: dict,
    ...
    # New tab -> YouTube Studio upload page
    pyautogui.hotkey("ctrl", "t")
    time.sleep(0.6)
    _paste("https://studio.youtube.com/channel/UClF-tM1X2bf2z_K5_Fd_Q5A/videos/short?d=ud&filter=%5B%5D&sort=%7B")
    time.sleep(0.2)
    pyautogui.press("enter")
    time.sleep(3.5)  # page load
    ...
    time.sleep(5.8)  # YouTube starts processing the upload; caption box appears
    ...
    # Title: the details dialog opens with the Title field focused and
    # auto-filled with the file name — select it all and replace it with the
    # requested title BEFORE tabbing down to the description field below.
    _title = (title or brief).strip()
    if _title:
...
def run(parameters: dict, player=None, session_memory=None) -> str:
    ...
    title = ((parameters or {}).get("title") or "").strip()
    print(f"[UploadVideo] title={title[:40]!r} brief={'<empty>' if not brief else brief[:40]!r}")
    if not brief:
    ...
    threading.Thread(target=_gen, daemon=True, name="youtube-caption").start()
    ...
    _log("JARVIS: YouTube upload starting — please don't touch the mouse "
         "or keyboard.")
    ...
    if not say:
        try:
            result = _run_upload_flow(title, brief, caption_holder, caption_ready)
        except Exception as e:
        ...
        return ("The video has been posted to YouTube successfully — announce "
    ...
    def _chatter():
        remarks = [
            ("[UPLOAD STATUS — speak now] The YouTube upload is underway in "
    ...
    def _worker():
        ...
        try:
            ...
            if result == "OK":
                ...
                _log(f"[UploadVideo] Posted to YouTube: \"{posted}...\"")
                say("[UPLOAD FINISHED — speak now] The video has just been "
                    "posted to YouTube successfully. Announce it to the user "
            ...
            print(f"[UploadVideo] Error: {e}")
            stop_chatter.set()
            say(f"[UPLOAD FAILED — speak now] The YouTube upload failed: {e}. "
    ...
    threading.Thread(target=_worker, daemon=True, name="youtube-upload").start()
    threading.Thread(target=_chatter, daemon=True, name="youtube-chatter").start()