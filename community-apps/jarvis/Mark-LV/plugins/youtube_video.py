...
PLUGIN_SETTINGS = {
    ...
    "title": "▶️  YOUTUBE",
    "fields": [
        {"key": "trending_region", "label": "Trending region (country code)",
         "type": "text", "default": "TR", "placeholder": "TR / US / GB / DE …"},
        {"key": "save_summaries", "label": "Always save video summaries to the Desktop",
...
def _handle_summarize(parameters: dict, player, speak) -> str:
    ...
    _save = parameters.get("save")
    save  = bool(_setting("save_summaries", False)) if _save is None else bool(_save)
    if save:
...
def _handle_trending(parameters: dict, player, speak) -> str:
    # "TR" used to be hardcoded here, which is only correct for one country.
    region = (parameters.get("region")
              or str(_setting("trending_region", "TR"))).strip().upper() or "TR"
...
def run(parameters: dict, player=None, session_memory=None) -> str:
    return youtube_video(parameters, player=player, speak=_speaker(player))
